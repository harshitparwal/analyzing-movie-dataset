...
def duplicate_and_unique_movies(dataset, index_):
    ...
    """
    dup=[dataset[i][index_] for i in range(len(dataset)) for j in range(i+1,len(dataset)) if dataset[i][index_]==dataset[j][index_]]
    return dup
...
dup=duplicate_and_unique_movies(movies,-2)
